...
from src.config.Config import img_size, batch_size, seed, number_aug
...
def load_dataset(train_dir,test_dir, augment=False,deeplearning=False,preprocess_fn=None):
    ...
    num_original = tf.data.experimental.cardinality(train_ds).numpy()*32
    print(f"Nombre total d'image train_ds originales : {num_original}")

    #compter le nombre total d'images originales
    num_original = tf.data.experimental.cardinality(val_ds).numpy()*32
    print(f"Nombre total d'image validation originales : {num_original}")

    #compter le nombre total d'images originales
    num_original = tf.data.experimental.cardinality(test_ds).numpy()*32
    ...
    if augment:
        ...
        num_augmented = tf.data.experimental.cardinality(train_ds).numpy()*32
...
def Balanced(train_ds):
    train_labels = np.concatenate(
        [y.numpy().ravel() for _, y in train_ds], axis=0).astype(int)
